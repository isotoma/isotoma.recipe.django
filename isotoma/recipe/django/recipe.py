...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        if self.options.has_key('extra-paths'):
            self.extra_paths = [path.strip() for path in self.options['extra-paths'].split('\n')]
        else:
            self.extra_paths = []
    ...
    def install_scripts(self, source_dir, project_dir):
        ...
        easy_install.scripts([('django-admin', "django.core.management", "execute_from_command_line")], ws, sys.executable, path, extra_paths = self.extra_paths)
        ...
        easy_install.scripts([(self.options['control-script'], 'django.core.management', 'execute_manager')], ws, sys.executable, path, arguments='settings', initialization="import %s.%s as settings" % (self.options['project'], self.options['settings']), extra_paths = self.extra_paths)
        ...
        if self.options['wsgi'].lower() == 'true':
            ...
            easy_install.scripts([(wsgi_name, 'isotoma.recipe.django.wsgi', 'main')], ws, sys.executable, path, arguments='settings', initialization="import %s.%s as settings" % (self.options['project'], self.options['settings']), extra_paths = [os.path.realpath(project_dir)] + self.extra_paths)
